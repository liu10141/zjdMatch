...
class Dock extends Model
{
    ...
    {
        $token = '';
        $post_data['account'] = config('you_ren_yun.account');
        $post_data['password'] = md5(config('you_ren_yun.password'));
        $post_url = config('you_ren_yun.API_ADDRESS') . '/user/login';
        $dataRes = http_json_data($post_url, $post_data, $token);
        $dataRes = json_decode($dataRes, true);
        return $dataRes;
    }
     ...
     * 获取项目列表
     */
    public function queryProjectList($data=array())
    {
     ...
     * 获取历史数据
     */
    public function getLastDataHistory($data)
    {
        $res = $this->interfaceDock('/user/regUser2', $data);
        return $res;
    }
