...
/**
 * Class Worker redis操作类，集成了redis常用的操作方法
 * Power by Mikkle
 * @time,2017/7/14
 */
class  RedisTools extends Controller
{
    public $redisObj = null;//redis实例化时静态变量 0
    ...
    public function __construct($options = [])
    {
        ...
        $auth = trim(isset($options["auth"]) ? $options["auth"] : "12345678");
    ...
    /**
     * @param $key
     * return length list长度
     * 查询list长度
     */
    public function leng($key)
    {
    ...
    {
        $re = $this->redisObj[$this->sn]->zcard($key);
//        halt($re);
        return $re;
    }
     ...
     * @param $key
     * @return mixed
     * 返回命令用于计算有序集合中指定分数区间的成员数量。
     */
    public function zcount($key, $min, $max)
    {
        $re = $this->redisObj[$this->sn]->zcount($key, $min, $max);
        return $re;
    }
